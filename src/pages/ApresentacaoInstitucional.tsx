...
const S11 = () => <Slide dark decor={<Grid dark />}><Header n="09" label="Case study • Korin / MyTS 360°" dark /><div className="mt-7 grid flex-1 grid-cols-[0.9fr_1.1fr] gap-11"><div className="overflow-hidden rounded-3xl"><img src={korinScene} alt="Korin packaging with QR code and digital experience" className="h-full w-full object-cover"/></div><div className="flex flex-col justify-center"><div className="flex w-fit items-center rounded-2xl bg-primary-foreground px-7 py-4"><Logo src={korinLogo} alt="Korin" h={66}/></div><h2 className="mt-7 font-display text-[51px] font-bold leading-[1.08] text-primary-foreground">From field to shelf, with the farmer in the spotlight.</h2><p className="mt-6 text-[24px] leading-[1.48] text-primary-foreground/80">A QR code on the packaging connects consumers to the product's origin through video, verified information and the farmer's own voice.</p><div className="mt-8 grid grid-cols-2 gap-5">{[["38","farmers"],["116","people in the field"],["13%","organic production"],["47%","family farming"]].map(([v,l])=><div key={l} className="rounded-2xl border border-primary-foreground/15 bg-primary-foreground/[0.07] p-6"><strong className="text-[39px] text-accent-glow">{v}</strong><span className="ml-3 text-[19px] text-primary-foreground/80">{l}</span></div>)}</div><a href="https://myt-s.com/korin/" target="_blank" rel="noreferrer" className="mt-7 inline-flex w-fit items-center gap-3 rounded-xl border border-primary-foreground/20 bg-primary-foreground/10 px-6 py-4 text-[20px] font-bold text-primary-foreground hover:bg-primary-foreground/20"><Globe2 size={22}/>myt-s.com/korin/</a></div></div></Slide>;

const S12 = () => <Slide><Header n="10" label="Trust at scale" /><Title size={48}>Results built within real supply chains.</Title><div className="mt-6 grid grid-cols-4 gap-5">{[[Building2,"2,000+","active companies"],[FileText,"500k+","documents"],[Workflow,"200+","active workflows"],[Globe2,"20+","countries"]].map(([I,v,l])=>{const Icon=I as typeof Building2;return <div key={String(l)} className="rounded-2xl border border-border bg-card p-6 shadow-card"><Icon className="text-accent" size={28}/><strong className="mt-5 block text-[38px] text-foreground">{String(v)}</strong><span className="text-[18px] text-muted-foreground">{String(l)}</span></div>})}</div><div className="mt-5 flex flex-1 flex-col justify-center rounded-3xl bg-secondary px-10"><p className="text-center text-[16px] font-bold uppercase text-muted-foreground" style={{letterSpacing:"0.14em"}}>Companies that already trust MyTS</p><div className="mt-5 grid grid-cols-4 items-center gap-4">{CLIENT_LOGOS.map(({name,src})=><div key={name} className="flex min-h-[78px] items-center justify-center rounded-xl border border-border bg-card px-4"><Logo src={src} alt={name} h={40}/></div>)}</div></div></Slide>;

const SMethod = () => <Slide dark decor={<><Grid dark/><Mark className="-bottom-36 -right-16 w-[560px]"/></>}><Header n="11" label="The MyTS methodology" dark/><Title dark>One complete cycle. One single platform.</Title><p className="mt-5 max-w-[1180px] text-[25px] leading-[1.45] text-primary-foreground/80">From supplier onboarding to continuous improvement, every stage feeds the next and keeps data, owners and evidence connected.</p><div className="mt-10 flex flex-1 items-center gap-5">{[[ShieldCheck,"Approval","Requirements, documents and initial clearance"],[Workflow,"Management","Processes, projects, deadlines and owners"],[Search,"Monitoring","Alerts, indicators and continuous control"],[TrendingUp,"Development","Action plans, progress and performance"]].map(([I,t,d],i,a)=>{const Icon=I as typeof ShieldCheck;return <Fragment key={String(t)}><div className="flex h-[330px] flex-1 flex-col justify-between rounded-3xl border border-primary-foreground/15 bg-primary-foreground/[0.07] p-8"><div className="flex items-center justify-between"><span className="grid size-16 place-items-center rounded-2xl bg-accent/15 text-accent-glow"><Icon size={34}/></span><span className="text-[18px] font-bold text-accent-glow">0{i+1}</span></div><div><h3 className="text-[29px] font-bold text-primary-foreground">{String(t)}</h3><p className="mt-3 text-[19px] leading-[1.4] text-primary-foreground/75">{String(d)}</p></div></div>{i<a.length-1&&<ArrowRight className="shrink-0 text-accent-glow" size={30}/>}</Fragment>})}</div><div className="rounded-2xl border border-accent/30 bg-accent/15 p-5 text-center text-[22px] font-semibold text-primary-foreground">No isolated systems, no rework and no lost history: the intelligence of the chain stays in one place.</div></Slide>;

const S13 = () => <Slide decor={<><Grid /><Mark className="-right-24 -bottom-36 w-[540px]" /></>}><Header n="12" label="How to start" /><div className="mt-9 grid flex-1 grid-cols-[0.9fr_1.1fr] items-center gap-11"><div><Pill>30-day free trial</Pill><h2 className="mt-8 font-display text-[61px] font-bold leading-[1.06] text-foreground">See the platform at work before any sales conversation.</h2><p className="mt-7 text-[25px] leading-[1.48] text-foreground/75">During the trial, you watch your suppliers use their own accounts and submit documents on their own.</p></div><div className="rounded-3xl bg-primary p-11"><p className="text-[17px] font-bold uppercase text-accent-glow" style={{letterSpacing:"0.14em"}}>Included in the MyTS plan</p><h3 className="mt-5 text-[36px] font-bold text-primary-foreground">Strategic Compliance Session</h3><p className="mt-4 text-[22px] leading-[1.45] text-primary-foreground/80">Two hours with a specialist to guide your operation on FSSC 22000, BRCGS, IFS, GMP, HACCP and supplier management.</p><div className="mt-8 space-y-4">{["Implementation support", "Setup of your first workflow", "Guidance throughout the trial"].map(x=><p key={x} className="flex items-center gap-3 text-[21px] text-primary-foreground"><CheckCircle2 className="text-accent-glow" size={24}/>{x}</p>)}</div></div></div></Slide>;

const S14 = () => <Slide dark decor={<><Grid dark /><Mark className="left-1/2 top-1/2 w-[720px] -translate-x-1/2 -translate-y-1/2" /></>}><div className="flex items-center justify-between"><Logo src={mytsLogo} alt="MyTS" h={50} invert/><Pill dark>Let's talk</Pill></div><div className="flex flex-1 flex-col items-center justify-center text-center"><h2 className="max-w-[1320px] font-display text-[68px] font-bold leading-[1.06] text-primary-foreground">Your team analyses.<br/><span className="text-accent-glow">MyTS keeps the chain moving.</span></h2><div className="mt-11 flex gap-6"><a href="mailto:tulio.santos@myt-s.com" className="flex items-center gap-3 rounded-xl bg-accent px-7 py-5 text-[22px] font-bold text-accent-foreground"><Mail size={25}/>tulio.santos@myt-s.com</a><a href="https://wa.me/5514991286962" className="flex items-center gap-3 rounded-xl border border-primary-foreground/20 bg-primary-foreground/10 px-7 py-5 text-[22px] font-bold text-primary-foreground"><MessageCircle size={25}/>(14) 99128-6962</a></div><p className="mt-9 text-[27px] font-semibold text-primary-foreground">Túlio dos Santos <span className="text-primary-foreground/55">• MyTS</span></p></div><div className="flex justify-between border-t border-primary-foreground/15 pt-5 text-[19px] text-primary-foreground/75"><span>myt-s.com</span><span>Botucatu, Brazil • Charlotte, USA</span></div></Slide>;
...
const ApresentacaoInstitucional = () => {
  ...
  return <main className="flex min-h-screen flex-col items-center bg-primary" style={{ padding: print ? 0 : "32px 0", gap: print ? 0 : 32, overflowX: "hidden" }}>
    <Helmet><title>MyTS — Company Presentation</title><meta name="description" content="MyTS company presentation: technology and consulting for continuous management of suppliers, processes and documents."/><meta property="og:title" content="MyTS — Company Presentation"/><meta property="og:description" content="Your entire supplier chain, without chasing anyone."/><meta property="og:type" content="website"/><meta name="twitter:card" content="summary_large_image"/></Helmet>
    ...
    {!print && <Button onClick={() => window.open(`${window.location.pathname}?print`, "_blank")} className="no-print fixed right-6 top-6 z-50 h-12 rounded-full bg-accent px-6 text-accent-foreground shadow-cta"><Download/>Download PDF</Button>}
