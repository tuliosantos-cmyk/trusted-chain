...
const S12 = () => <Slide><Header n="10" label="Confiança em escala" /><Title>Resultados construídos em cadeias reais.</Title><div className="mt-9 grid grid-cols-4 gap-6">{[[Building2,"2.000+","empresas ativas"],[FileText,"500 mil+","documentos"],[Workflow,"200+","fluxos ativos"],[Globe2,"20+","países"]].map(([I,v,l])=>{const Icon=I as typeof Building2;return <div key={String(l)} className="rounded-2xl border border-border bg-card p-8 shadow-card"><Icon className="text-accent" size={35}/><strong className="mt-7 block text-[48px] text-foreground">{String(v)}</strong><span className="text-[21px] text-muted-foreground">{String(l)}</span></div>})}</div><div className="mt-9 flex flex-1 flex-col justify-center rounded-3xl bg-secondary px-10"><p className="text-center text-[17px] font-bold uppercase text-muted-foreground" style={{letterSpacing:"0.14em"}}>Empresas que já confiam na MyTS</p><div className="mt-9 grid grid-cols-6 items-center gap-5">{CLIENT_LOGOS.map(({name,src})=><div key={name} className="flex min-h-[104px] items-center justify-center rounded-xl border border-border bg-card px-4"><Logo src={src} alt={name} h={52}/></div>)}</div></div></Slide>;
