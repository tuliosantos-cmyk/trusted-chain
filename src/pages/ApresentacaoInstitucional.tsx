...
const S07 = () => <Slide><Header n="05" label="Módulo 02" /><div className="mt-5 grid flex-1 grid-cols-[0.7fr_1.3fr] items-center gap-10"><div><Pill>Meus Processos</Pill><h2 className="mt-7 font-display text-[55px] font-bold leading-[1.08] text-foreground">A rotina da qualidade, sem planilha solta.</h2><p className="mt-6 text-[25px] leading-[1.45] text-foreground/75">Autoavaliações, checklists e registros automatizados, configurados de acordo com o padrão de cada empresa.</p></div><Window title="Fluxo de autoavaliação"><div className="p-8"><div className="flex items-center justify-between"><div><p className="text-[17px] text-muted-foreground">Processo ativo</p><h3 className="text-[31px] font-bold text-foreground">Qualificação anual de fornecedor</h3></div><Pill>76% concluído</Pill></div><div className="mt-9 space-y-5">{[[Check,"Convite enviado","Concluído"],[ClipboardCheck,"Autoavaliação respondida","Concluído"],[Search,"Validação técnica","Em análise"],[FileCheck2,"Plano de ação","Próxima etapa"]].map(([I,t,s],i)=>{const Icon=I as typeof Check; return <div key={String(t)} className="flex items-center gap-5"><span className={`grid size-16 place-items-center rounded-full ${i<2?"bg-success text-success-foreground":i===2?"bg-accent text-accent-foreground":"bg-secondary text-muted-foreground"}`}><Icon size={27}/></span><div className="flex-1 rounded-xl border border-border p-5"><div className="flex justify-between"><strong className="text-[21px] text-foreground">{String(t)}</strong><span className="text-[16px] text-muted-foreground">{String(s)}</span></div></div></div>})}</div></div></Window></div></Slide>;
